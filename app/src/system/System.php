...
class System
{
    ...
    public function addDependant(){
        ...
        try{
            ...
            $check = "SELECT COUNT(*) FROM `dependant` WHERE `national_ID` = '$nID'";

            if ($res = $this->pdo->query($check)) {

                if ($res->fetchColumn() !=0) {
                    $data = array('success' => false, 'statusCode' => FORBIDEN,'message'=> 'Dependant National ID number already exists');
                    return $data;
                }else {
                    $mnumber = new MembershipNumber();
                    $mnumber->setId($member);
                    $memberNumber = $mnumber->dependantNumber();
                    $sql = "INSERT INTO `dependant`(`id`, `member_id`,`membership_no`, `name`, `surname`, `national_ID`, `dob`, `gender`, `created`, `updated`) VALUES ('','$member','$memberNumber','$name','$sname','$nID','$dob','$sex', now(),'')";
                    $insert = mysqli_query($this->con, $sql);
                    if ($insert){
                        $data = array('success' => true, 'statusCode' => SUCCESS_RESPONSE,'message'=> 'Dependant added successfully');
                        return $data;
                    }else{
                        $data = array('success' => false, 'statusCode' => INTERNAL_SERVER_ERROR, 'error'=> array('type' => 'SERVER_ERROR', 'message' => 'Could not save dependant'));
                        return $data;
                    }
                }
            }else {
            ...
            $data = array('success' => false, 'statusCode' => INTERNAL_SERVER_ERROR, 'error'=> array('type' => 'PROCESS_SERVER_ERROR', 'message' => $exception->getMessage()));
            return $data;
        }
    }
